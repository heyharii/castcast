...
class UserTest extends TestCase
{
   ...
   public function test_can_get_all_series_being_watched_by_user(){
       ...
       //collection of the series
        ...
        $this->assertInstanceOf(\Castcast\Series::class, $startedSeries->random());

        $idOfStartedSeries = $startedSeries->pluck('id')->all();

        $this->assertTrue(
            in_array($lesson->series->id, $idOfStartedSeries)
        );

        $this->assertTrue(
            in_array($lesson3->series->id, $idOfStartedSeries)
        );

        $this->assertFalse(
            in_array($lesson6->series->id, $idOfStartedSeries)
        );
